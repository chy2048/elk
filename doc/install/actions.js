...
export default function (server, actions, payload, watch) {
  ...
  if (config.settings.email.active) {
   ...
   /* server,log(['status', 'info', 'Sentinl', 'email'],"***********"+config.settings.email.user); */
	server.log(['status', 'info', 'Sentinl', 'email'],'**********emial user');
	server.log(['status', 'info', 'Sentinl', 'email'],config);
    ...
    */
    ...
    if (_.has(action, 'webhook')) {
      ...
      options = {
        ...
        path: action.webhook.path ? mustache.render(action.webhook.path,{payload:payload}) : '/',
	  ...
	  {
		dataToWrite = "can not be nil";
        options.headers['Content-Length'] = Buffer.byteLength(dataToWrite);
	  }

// 	  server.log(['status', 'debug', 'Sentinl','Webhook, mail content = '],  mustache.render(options.path));
      // Log Alarm Event
...
//	  server.log(['status', 'debug', 'Sentinl'], 'Webhook exceute!!!!');
